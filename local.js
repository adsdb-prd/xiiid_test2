...
(() => {
  const localUrl = input => {
    ...
    if (url.pathname.includes('154adc4df997f6b1fe8680543e03d926ec51543b-1200x630')) return new URL('/brand/hero-share.jpg', location.origin).href;
    if (url.hostname.endsWith('.api.sanity.io')) return new URL('/content/home-response.json?v=16', location.origin).href;
  ...
  addEventListener('DOMContentLoaded', () => {
    ...
    // Keep hydrated SEO consistent with the crawlable HTML share card.
    const syncShare = () => {
      const values = {
        'og:title': 'XIIID AI Labs', 'twitter:title': 'XIIID AI Labs',
        'description': 'Education Without Borders, XIIID AI Labs',
        'og:description': 'Education Without Borders, XIIID AI Labs',
        'twitter:description': 'Education Without Borders, XIIID AI Labs',
        'og:url': 'https://adsdb-prd.github.io/xiiid_test2/',
        'og:image': 'https://adsdb-prd.github.io/xiiid_test2/brand/hero-share.jpg',
        'twitter:image': 'https://adsdb-prd.github.io/xiiid_test2/brand/hero-share.jpg'
      };
      document.head.querySelectorAll('meta').forEach(meta => {
        const value = values[meta.getAttribute('property') || meta.name];
        if (value && meta.content !== value) meta.content = value;
      });
    };
    syncShare();
    new MutationObserver(syncShare).observe(document.head, {subtree:true,childList:true,attributes:true,attributeFilter:['content']});
