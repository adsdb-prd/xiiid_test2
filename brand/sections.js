...
(function () {
  ...
  var DEPLOYMENTS = [
    ...
    {
      place: 'Paraguay', product: 'National rollout', state: 'In progress', href: 'https://xdotup-dev.web.app/', button: 'Open Beta',
  ...
  function buildNav(header) {
    ...
    var cta = el('a', {
      class: 'x-nav-cta', href: '#apps', text: 'APP'
    });
  ...
  function ecosystemSection() {
    ...
    var field = el('ul', { class: 'x-field' }, DEPLOYMENTS.map(function (d) {
      return el('li', { class: 'x-field-row' }, [
        ...
        el('div', { class: 'x-field-body' }, [
          ...
          d.href ? extLink(d.href, 'x-service-link', d.button || 'Open ' + d.product) : null
        ])
      ]);
    }));
    ...
    var clsCol = el('div', { class: 'x-map-col x-map-class' }, trackHead(TRACKS.cls).concat([
      ...
      el('div', { class: 'x-map-cta-row' }, [el('a', { href: '#apps', class: 'x-studio-cta', text: 'Open XClass' })])
    ]));
    ...
    var people = el('div', { class: 'x-team-grid' }, TEAM.map(function (m) {
      return el('article', { class: 'x-team-card' }, [
        ...
        el('div', { class: 'x-team-body' }, [
          ...
          m.linkedin ? extLink(m.linkedin, 'x-service-link', 'LinkedIn') : null
        ])
      ]);
    }));
  ...
  function roadmapSection() {
    var phases = [
      { year: '2025', title: 'Building the foundation', items: [
        'XIIID project launch · February 6', 'Modularization of AI Studio',
        'Preparation for ENEM service in Brazil', 'Development of the AI Class platform'
      ] },
      { year: '2026', title: 'Bringing learning to life', items: [
        'Preparation and launch of AI Class service in Paraguay',
        'Paraguay internship in the US · June 6–22',
        'XClass web / app beta · June 14; web launch · June 26',
        'XWallet app beta · June 28; XClass iOS / Android launch · June 30',
        'XWallet iOS / Android official launch', 'Preparation for ENEM service in Brazil'
      ] },
      { year: '2027', title: 'Expanding access', items: [
        'Preparation and launch of SAEB service in Brazil', 'Launch of ENEM service in Brazil',
        'Preparation and launch of online education certification',
        'Expansion to international exams: GED, IELTS and TOEFL', 'Advancement of AI Studio'
      ] },
      { year: '2028', title: 'A new kind of school', items: [
        'Preparation and launch of Online School', 'Preparation for Metaverse School service'
      ] }
    ];
    return el('section', { class: 'x-section x-roadmap', id: 'roadmap', 'aria-labelledby': 'roadmap-title' }, [
      el('div', { class: 'x-inner' }, [
        el('div', { class: 'x-head' }, [
          el('h2', { class: 'x-title', id: 'roadmap-title', text: 'Roadmap.' })
        ]),
        el('ol', { class: 'x-roadmap-grid' }, phases.map(function (phase) {
          return el('li', { class: 'x-roadmap-phase' }, [
            el('div', { class: 'x-roadmap-marker' }, [
              el('span', { class: 'x-roadmap-year', text: phase.year }),
              el('span', { class: 'x-roadmap-period', text: 'Q1–Q4' })
            ]),
            el('h3', { class: 'x-roadmap-name', text: phase.title }),
            el('ul', { class: 'x-roadmap-items' }, phase.items.map(function (item) { return el('li', { text: item }); }))
          ]);
        }))
      ])
    ]);
